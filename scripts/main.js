...
game.canvasPlaceItem = (e) => {
	if (game.building) {
		const mouseX = e.clientX - e.target.offsetLeft;
		const mouseY = e.clientY - e.target.offsetTop;
		const x = Math.floor(mouseX / game.tileWidth);
		const y = Math.floor(mouseY / game.tileHeight);
		game.placingWall = [x, y];
	}
}
...
game.getElements = () => {
	...
	game.build.menu.wall = document.querySelector('#buildWall');
	game.build.menu.wall.addEventListener('click', game.build.wall);
  ...
  game.canvasGUI = document.querySelector('#guiCanvas');
	...
	game.canvasGUI.addEventListener("mousemove", game.canvasPlaceItem);
...
game.build.wall = () => {
	if (game.base.inventory.stone >= 5) {
		game.base.inventory.stone -= 5;
		game.updateDisplay();
		game.building = true;
	}
}
...
game.drawGUI = () => {
	...
	for (let x = 0; x < game.worldWidth; x++) {
		for (let y = 0; y < game.worldHeight; y++) {
			...
			if (x === game.placingWall[0] && y === game.placingWall[1]) {
				spriteNum = 7;
			}
